...
import { Pane, LayoutSet, ThemeProvider } from "fannypack"
...
stories.add("with paragraph select", () => (
  <Textbox text={text} select="paragraph" />
))
stories.add("with word select", () => (
  <Textbox text={text} select="word" />
))
stories.add("with swimlanes", () => (
  <LayoutSet>
    <Textbox text={text.slice(0, 1)} select="word" swimlanes />
    <Textbox text={text.slice(0, 1)} select="word" swimlanes secondLane />
    <Textbox text={text.slice(0, 1)} select="word" swimlanes secondLane />
  </LayoutSet>
))
...
export const processParagraphs = paragraphsText => {
  ...
  const paragraphsData = paragraphs.map((paragraphText, index) => {
    ...
    const wordsData = words.map((wordText, index) => ({
      ...
      index,
      text: wordText,
      text2: wordText.split("").reverse().join(""),
      style: {},
    }))
    return {
      type: "paragraph",
